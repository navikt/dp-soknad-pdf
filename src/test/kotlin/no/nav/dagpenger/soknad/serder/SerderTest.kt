...
internal class SerderTest {
    ...
    @Test
    fun `Mapper fra json til Seksjoner og SpmOgSvar`() {
        Oppslag(tekstJson).also {
            assertEquals("f3", it.lookup("f3").textId)
            assertEquals("seksjon1", it.lookup("seksjon1").textId)
        }
    }
}
